...
# { $runtimeVersion } is the version of the remote browser (for instance "67.0a1")
about-debugging-browser-version-too-old-67-debugger = Панель отладчика может не работать с подключённым браузером. Пожалуйста, используйте Waterfox { $runtimeVersion }, если вам необходимо использовать отладчик с этим браузером.
