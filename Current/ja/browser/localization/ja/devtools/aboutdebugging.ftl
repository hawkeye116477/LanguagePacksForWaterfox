...
# USB section step by step guide
about-debugging-setup-usb-step-enable-debug-firefox2 = Android 端末の Waterfox の USB デバッグを有効にします。
...
# { $runtimeVersion } is the version of the remote browser (for instance "67.0a1")
about-debugging-browser-version-too-old-67-debugger = 接続したブラウザーではデバッガーパネルが動作しないようです。このブラウザーでデバッガーを利用したい場合は、 Waterfox { $runtimeVersion } を使用してください。
