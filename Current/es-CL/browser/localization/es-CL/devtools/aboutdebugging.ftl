...
# USB section step by step guide
about-debugging-setup-usb-step-enable-debug-firefox2 = Activa la depuración USB en Waterfox en el dispositivo Android.
# USB section step by step guide
...
# { $runtimeVersion } is the version of the remote browser (for instance "67.0a1")
about-debugging-browser-version-too-old-67-debugger = Puede que el panel del depurador no funcione con el navegador conectado. Por favor, usa Waterfox { $runtimeVersion } si necesitas usar el depurador con este navegador.
...
about-debugging-browser-version-too-recent = El navegador conectado es más reciente ({ $runtimeVersion }, buildID { $runtimeID }) que tu { -brand-shorter-name } ({ $localVersion }, buildID { $localID }). Esta es una configuración no soportada y puede hacer que las herramientas de desarrollo fallen. Por Favor, actualiza Waterfox. <a>Troubleshooting</a>
